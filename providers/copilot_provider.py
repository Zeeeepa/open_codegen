"""Copilot Provider - Stub implementation"""
import asyncio
import logging
...
class CopilotProvider(BaseProvider):
    async def initialize(self):
        ...
        logger.info("✅ Copilot provider initialized (stub)")
    ...
    async def process_request(self, webchat_request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response_content = f"[COPILOT STUB] Response to: {message}"
            self._record_request_success(start_time)
            return self._create_success_response(response_content, {"model": "copilot", "provider": "copilot"})
        except Exception as e:
            error_msg = f"Copilot request failed: {str(e)}"
