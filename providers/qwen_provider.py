"""Qwen Provider - Stub implementation"""
import asyncio
import logging
...
class QwenProvider(BaseProvider):
    async def initialize(self):
        ...
        logger.info("✅ Qwen provider initialized (stub)")
    ...
    async def process_request(self, webchat_request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response_content = f"[QWEN STUB] Response to: {message}"
            self._record_request_success(start_time)
            return self._create_success_response(response_content, {"model": "qwen", "provider": "qwen"})
        except Exception as e:
            error_msg = f"Qwen request failed: {str(e)}"
