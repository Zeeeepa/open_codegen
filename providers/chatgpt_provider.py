"""ChatGPT Provider - Stub implementation"""
import asyncio
import logging
...
class ChatGPTProvider(BaseProvider):
    async def initialize(self):
        ...
        logger.info("✅ ChatGPT provider initialized (stub)")
    ...
    async def process_request(self, webchat_request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response_content = f"[CHATGPT STUB] Response to: {message}"
            self._record_request_success(start_time)
            return self._create_success_response(response_content, {"model": "chatgpt", "provider": "chatgpt"})
        except Exception as e:
            error_msg = f"ChatGPT request failed: {str(e)}"
